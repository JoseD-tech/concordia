    ...
    <div :class="$page.props.showingMobileMenu ? 'translate-x-0 ease-out' : '-translate-x-full ease-in'"
        ...
        <div class="flex justify-center items-center mt-8">
            <div class="flex items-center">
                ...
                <span class="mx-2 text-2xl font-semibold text-white">Concordia</span>
            </div>
        </div>
        ...
        <nav class="mt-10" x-data="{ isMultiLevelMenuOpen: false }">
            ...
            <nav-link :href="route('participantes')" :active="route().current('participantes')">
                <template #icon>
            ...
            <a class="flex items-center mt-4 py-2 px-6 text-gray-100" href="#" @click="showingTwoLevelMenu = !showingTwoLevelMenu">
                ...
                <span class="mx-3">Configuracion</span>
            </a>
            <transition
                ...
                <div v-show="showingTwoLevelMenu">
                    <ul class="overflow-hidden p-2 mx-4 mt-2 space-y-2 text-sm font-medium text-white bg-gray-700 bg-opacity-50 rounded-md shadow-inner"
                        ...
                        <li class="px-2 py-1 transition-colors duration-150">
                            <Link class="w-full" :href="route('cursos.index')">Registrar Cursos</Link>
                        </li>
                        <li class="px-2 py-1 transition-colors duration-150">
                            <Link class="w-full" :href="route('dashboard')">Registrar Profesores</Link>
                        </li>
                        <li class="px-2 py-1 transition-colors duration-150">
                            <Link class="w-full" :href="route('dashboard')">Registrar Sedes</Link>
                        </li>
                    </ul>
                </div>
            </transition>
